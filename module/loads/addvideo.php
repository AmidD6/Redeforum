...
if ($_POST['enter'] and $_POST['text'] and $_POST['name'] and $_POST['cat']) {
	...
	if (!$_FILES['video']['type']) MessageSend(2, 'Ви не вибрали відео.');
...
foreach(glob('catalog/video/loads/*', GLOB_ONLYDIR) as $num => $Dir) {
$num_video ++;
...
if ($_SESSION['ACTIVE_VIDEO'] >= $_SESSION['CONST_VIDEO']) MessageSend(2, 'На даний момент відсутня можливість добавляти зображення.', '/loads');
}
else $num_video = 0;
...
if ($_FILES['img']['type']){
foreach(glob('catalog/img/loads/mini/*', GLOB_ONLYDIR) as $num => $Dir) {
$num_img ++;
$Count = sizeof(glob($Dir.'/*.*'));
if ($Count < 250) {
...
//MiniIMG('catalog/img/loads/'.$num_img.'/'.$MaxId[0].'.jpg', 'catalog/img/loads/mini/'.$num_img.'/'.$MaxId[0].'.jpg', 465, 300);
...
mysqli_query($CONNECT, "INSERT INTO `loads`  VALUES ($MaxId[0], '$_SESSION[CATY_VAL]', '$_POST[name]', '$_POST[cat]', 0, 0, '$_SESSION[USER_LOGIN]', '$_POST[text]', NOW(), $Active, 0, $num_file, $num_img, $num_video, 0, '', '', '$_SESSION[USER_ID]', 0)");
...
<br>
<form method="POST" action="/loads/addvideo" enctype="multipart/form-data">
...
<br>
  ...
  <ul class="nav nav-tabs nav-justified">
   <li class="nav-item">
      <a class="nav-link active" data-toggle="tab"  style="color: #5d0c73; text-shadow: 1px 1px 10px #000;"href="#home">Відео</a>
    </li>
    <li class="nav-item">
      <a class="nav-link" data-toggle="tab" style="color: #5d0c73; text-shadow: 1px 1px 10px #000;" href="#menu1">Зображення</a>
    </li>
    <li class="nav-item">
...
<input type="submit" style="text-shadow: 1px 1px 10px #000;" name="enter" value="Створити"> <input type="reset" style="text-shadow: 1px 1px 10px #000;" value="Знищити"><a href="#"style="float: right; color: #fff; text-shadow: 1px 1px 10px #ff0000;">Як добавити тему ?</a>
</form>
<br>
</div>
</div>
